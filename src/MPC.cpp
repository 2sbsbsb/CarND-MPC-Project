  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // the Solver function below.
     ...
     fg[0] = 0;
      ...
      // Minimize the use of actuators and penalize speed + steer
      ...
      for (unsigned int t = 1; t < N; t++) {
          ...
          if (t > 1) {   // use previous actuation (to account for latency)
              a = vars[a_start + t - 2];
...
// Set the initial variable values
    ...
    // The upper and lower limits of delta are set to -25 and 25 degrees (values are in radians)
  ...
  vector<double> result;
    ...
    result.push_back(solution.x[delta_start]);   //first value is steer
    result.push_back(solution.x[a_start]);       //second value is throttle
